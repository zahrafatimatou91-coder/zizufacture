...
import { updateInvoice, getInvoiceById } from "@/lib/actions/invoices";
...
export default function EditInvoicePage({ params }: { params: { id: string } }) {
  ...
  const [invoiceDisplayId, setInvoiceDisplayId] = useState<string>("");
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const [cliData, settsData, invData] = await Promise.all([
          getClients(), 
          getSettings(),
        ...
        if (invData) {
          setClientId(invData.client_id);
          setIssueDate(new Date(invData.issue_date));
          setDueDate(new Date(invData.due_date));
          setInvoiceDisplayId(invData.display_id);
          
          if (invData.invoice_lines && invData.invoice_lines.length > 0) {
            setLines(invData.invoice_lines.map((l: any) => ({
            ...
            setLines([{ id: Date.now().toString(), description: "", quantity: 1, unitPrice: "" }]);
          }
        }
      } catch (error) {
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 p-2 sm:p-0">
          ...
          <Card className="w-full max-w-md p-6 rounded-2xl shadow-2xl border-0 animate-in zoom-in-95 relative">
            ...
            <div className="flex flex-col items-center text-center space-y-4">
              ...
              <div className="w-full pt-4 space-y-3">
                ...
                <Button 
                  onClick={() => router.push(`/dashboard/factures/${decodedId}`)}
                  variant="outline"
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 print:hidden">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-slate-900">Modifier la Facture</h1>
            <p className="text-slate-500 text-sm mt-1">Mettez à jour les informations de la facture {invoiceDisplayId}.</p>
          </div>
        </div>
        <div className="flex gap-3 w-full sm:w-auto">
            ...
            <div className="relative z-10 flex-1 flex flex-col">
              <div className="flex justify-between items-start mb-10">
                ...
                <div className="text-right">
                  ...
                  <p className="text-slate-400 text-xs sm:text-sm mt-1 font-medium">{invoiceDisplayId || "Brouillon"}</p>
                </div>
              </div>
